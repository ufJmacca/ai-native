...
class EventSink:
    """Collect canonical RunnerEvent lines and finalize one immutable stream."""
    ...
    def append(self, event: RunnerEvent) -> None:
        ...
        with self._lock:
            ...
            if self._identity is not None and identity != self._identity:
                raise ValueError("event identity must remain constant within one stream")
    ...
    def finalize(self) -> ArtifactReference:
        ...
        with self._lock:
            self._ensure_open()
            reference = self._writer.write_bytes(
                EVENT_STREAM_PATH,
                bytes(self._content),
